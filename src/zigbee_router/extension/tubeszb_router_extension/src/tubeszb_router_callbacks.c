/***************************************************************************//**
 * @file tubeszb_router_callbacks.c
...
static void commissioning_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  if (sl_zigbee_af_network_state() != SL_ZIGBEE_JOINED_NETWORK) {
    ...
    if (status != SL_STATUS_OK) {
      ...
      sl_zigbee_af_event_set_delay_ms(event, NWK_STEERING_COOLDOWN_MS);
    }
  }
}
 ...
 */
void tubeszb_router_stack_status_cb(sl_status_t status)
{
 ...
 */
void tubeszb_router_init(uint8_t init_level)
{
  (void)init_level;
...
void sl_button_on_change(const sl_button_t *handle)
{
  ...
  if (button_state == SL_SIMPLE_BUTTON_PRESSED) {
    ...
    button_long_press = (uint16_t)(current_time - button_press_time) > BUTTON_LONG_PRESS_TIME_MS;
